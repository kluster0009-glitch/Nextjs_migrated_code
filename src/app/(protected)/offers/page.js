...
import { SoftwareCard } from "@/components/offers/SoftwareCard";

/**
 * Student Software Hub - Offers Page
 * 
 * Displays free software tools and benefits available to students.
 * Features search functionality and responsive grid layout.
 * 
 * All benefits data is stored in the studentBenefits array for easy expansion.
 */
export default function OffersPage() {
  const [searchQuery, setSearchQuery] = useState("");

  // Student Benefits Data - Easy to expand in the future
  ...
  const filteredBenefits = studentBenefits.filter((benefit) => {
    ...
    return (
      ...
      benefit.eligibility.toLowerCase().includes(query) ||
      benefit.category.toLowerCase().includes(query) ||
      (Array.isArray(benefit.benefits)
        ? benefit.benefits.some((b) => b.toLowerCase().includes(query))
        : benefit.benefits.toLowerCase().includes(query))
    );
  });
      ...
      <div className="w-full py-6 px-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-lg">
            Explore all software, tools, and perks you can access with your student email.
          </p>
        </div>
            ...
            <Card className="border-cyber-border bg-cyber-card/50 backdrop-blur-xl mb-6">
              <CardContent className="p-4">
                <div className="relative">
                  ...
                  <Input
                    type="text"
                    placeholder="Search by software name, category, or benefit..."
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                    className="pl-12 bg-cyber-darker/50 border-cyber-border focus:border-neon-cyan text-base"
                  />
                </div>
              </CardContent>
            </Card>
            ...
            {searchQuery && (
              <div className="mb-4">
                <p className="text-sm text-muted-foreground">
                  Found <span className="text-neon-cyan font-semibold">{filteredBenefits.length}</span> {filteredBenefits.length === 1 ? 'benefit' : 'benefits'}
              ...
              <Card className="border-cyber-border bg-cyber-card/50 backdrop-blur-xl">
                <CardContent className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground">
                    Try adjusting your search query or browse all available benefits.
                  </p>
                </CardContent>
              </Card>
            )}
            ...
            <Card className="border-cyber-border bg-cyber-card/50 backdrop-blur-xl sticky top-6">
              <CardContent className="p-6">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  We're constantly adding new student benefits and opportunities. Check back regularly!
                </p>
                <div className="space-y-3">
                  <div className="p-3 rounded-lg bg-cyber-darker/50 border border-cyber-border">
                    <p className="text-sm font-medium text-foreground mb-1">
                      🎓 Coming Next
                    </p>
                    <p className="text-xs text-muted-foreground">
                      Adobe Creative Cloud, Spotify Premium, Apple Music, and more!
                    </p>
                  </div>
                  <div className="p-3 rounded-lg bg-cyber-darker/50 border border-cyber-border">
                    <p className="text-sm font-medium text-foreground mb-1">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Know a great student offer? Let us know in the feedback section.
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent className="p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Total Benefits</span>
                    <span className="text-xl font-bold text-neon-cyan">
                      {studentBenefits.length}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Categories</span>
                    <span className="text-xl font-bold text-neon-purple">
                      {new Set(studentBenefits.map((b) => b.category)).size}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Cloud Services</span>
                    <span className="text-xl font-bold text-neon-cyan">
                      {studentBenefits.filter((b) => b.category === "Cloud").length}
                    </span>
                  </div>
                </div>
              </CardContent>
            ...
            <Card className="border-cyber-border bg-cyber-card/50 backdrop-blur-xl">
              <CardContent className="p-6">
                ...
                <div className="space-y-2">
                  {Array.from(new Set(studentBenefits.map((b) => b.category)))
                    .sort()
                    .map((category) => {
                      const count = studentBenefits.filter(
                      ...
                      return (
                        <button
                          ...
                          className="w-full flex items-center justify-between p-2 rounded-lg hover:bg-cyber-darker/50 transition-colors text-left"
                        >
                          <span className="text-sm text-foreground">{category}</span>
                          <span className="text-xs text-muted-foreground bg-cyber-darker/70 px-2 py-1 rounded">
                            {count}
                          </span>
                        </button>
                      );
